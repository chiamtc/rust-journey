...
impl Parser {
    ...
    fn consume_char(&mut self) -> char {
        ...
        let (_, cur_char) = iter.next().unwrap(); //first item = index, second item is the actual character in tuple
        let (next_pos, _) = iter.next().unwrap_or((1, ' '));
        self.pos += next_pos;
        cur_char
    }
    fn consume_while<F>(&mut self, cond: F) -> String where F: Fn(char) -> bool, {
        let mut result = String::new();
